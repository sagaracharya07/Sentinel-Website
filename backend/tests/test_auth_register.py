...
def test_register_ignores_client_supplied_role(app, client):
    with patch("app.send_email", return_value=True):
        client.post("/api/auth/register", json={
            "username": "wannabe_admin", "email": "wa@example.com",
...
def test_verify_email_rejects_expired_token(app, client):
    ...
    with app.app_context():
        user = User.query.filter_by(username="alice").first()
        user.verification_token_expires = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=1)
        db.session.commit()
...
def test_forgot_password_same_response_regardless_of_email_existing(app, client):
    with patch("app.send_email", return_value=True):
        _register(client)
        resp_exists = client.post("/api/auth/forgot-password", json={"email": "alice@example.com"})
        resp_missing = client.post("/api/auth/forgot-password", json={"email": "nobody@example.com"})
...
def test_reset_password_full_cycle(app, client):
    ...
    resp = client.post("/api/auth/reset-password", json={"token": token, "password": "newpassword456"})
    assert resp.status_code == 200

    resp = client.post("/api/auth/login", json={"username": "alice", "password": "newpassword456"})
    assert resp.status_code == 200

    # token is single-use
    resp = client.post("/api/auth/reset-password", json={"token": token, "password": "again12345678"})
    assert resp.status_code == 400
...
def test_reset_password_rejects_weak_new_password(app, client):
    ...
    with app.app_context():
        token = User.query.filter_by(username="alice").first().reset_token
    resp = client.post("/api/auth/reset-password", json={"token": token, "password": "weak"})
    assert resp.status_code == 400


def test_change_password_requires_correct_current_password(user_client):
    resp = user_client.post("/api/auth/change-password", json={
...
def test_change_password_success(user_client):
    resp = user_client.post("/api/auth/change-password", json={
        "current_password": "test_password_123", "new_password": "newpassword456",
    })
    assert resp.status_code == 200
    user_client.post("/api/auth/logout")
    resp = user_client.post("/api/auth/login", json={"username": "test_user", "password": "newpassword456"})
    assert resp.status_code == 200
