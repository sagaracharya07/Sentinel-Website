...
def seed_model_version_row():
    ...
    if db.session.get(ModelVersion, version):
        return
    m = info["metrics"]
    meta = info["meta"]
...
SAMPLE_EMAILS = [
    ...
    dict(
        subject="Invoice #4471 from A1 Ultimate Roofing",
    ...
    dict(
        subject="Re: Roofing materials quote",
        sender="sales@colorbondsuppliesau.com",
        body="Hi Sagar, please find attached the updated Colorbond pricing for the Riverwood "
        "job as discussed on the phone.",
        offset_hours=50,
        created_by="user",
    ),
    dict(
        subject="Final Notice: Payment failed, reactivate your account",
